...
class EventDetection:
    ...
    def detect_objects(self, image_or_frame):
        """
        Detects objects in an image or video frame using YOLO model.

        Args:
            image_or_frame (str or np.array): Either the path to the image file or a video frame.

        Returns:
            list: List of detected objects, each represented by a tuple of class label and bounding box coordinates.
        """
        try:
            if isinstance(image_or_frame, str):  # If the input is an image path
                # Load the image using OpenCV
                image = cv2.imread(image_or_frame)
            else:  # If the input is a video frame
